...
with st.sidebar:
    ...
    page_values = ["Home", "Market Prices", "Crop Recommendation", "Government Schemes", "Financial Assistant", "Business Recommendation", "Smart Action Plan", "Rural Intelligence Hub"]
    page_labels = {
        "Home": "🏠 " + tr("home"),
...
# ============================================================
# CROP RECOMMENDATION — KAGGLE + SOILFER REFERENCE
# ============================================================
if page == "Crop Recommendation":
    page_banner(
        "🌱",
        "Crop Recommendation",
        "Use the Kaggle Crop Recommendation Dataset with your soil and climate inputs."
    )

    st.info(
        "The recommendation engine below uses the public Kaggle dataset as its training/reference data. "
    ...
    if df_crop is None:
        ...
        with c1:
            n_val = st.number_input("Nitrogen (N)", min_value=0.0, max_value=200.0, value=50.0, step=1.0)
            p_val = st.number_input("Phosphorus (P)", min_value=0.0, max_value=200.0, value=50.0, step=1.0)
            k_val = st.number_input("Potassium (K)", min_value=0.0, max_value=250.0, value=50.0, step=1.0)
        with c2:
            temp_val = st.number_input("Temperature (°C)", min_value=-10.0, max_value=60.0, value=25.0, step=0.5)
            humidity_val = st.number_input("Humidity (%)", min_value=0.0, max_value=100.0, value=70.0, step=1.0)
        with c3:
            ph_val = st.number_input("Soil pH", min_value=0.0, max_value=14.0, value=6.5, step=0.1)
            rain_val = st.number_input("Rainfall (mm)", min_value=0.0, max_value=5000.0, value=100.0, step=5.0)
