...
if page == "Market Prices":
    ...
    for start in range(0, len(items), 4):
        ...
        for col, (crop_name, data) in zip(cols, items[start:start + 4]):
            with col:
                ...
                local_tag = " • Local crop" if crop_name in local_crop_names else ""
                st.markdown(f"""
                <div class="card">
    ...
    if filtered:
        ...
        if crop in local_crop_names:
            st.success(f"🌱 {tr_data(crop)} is included in the prototype's local-crop profile for **{location}**.")
        else:
            st.info(f"ℹ️ {tr_data(crop)} is shown because you selected **{tr('show_all_crops')}**.")
    else:
        st.warning("No crop is available for this category in the current prototype profile.")
...
if page == "Government Schemes":
    ...
    for scheme in shown:
        with st.expander(scheme_name(scheme["name"])):
            ...
            if scheme_details:
                st.markdown(f"**{tr('eligibility')}:** {scheme_details['eligibility']}")
...
if page == "Business Recommendation":
    ...
    with col2:
        ...
        st.info(f"💧 Water availability for **{location}**: **{water_labels[local_water]}**")
    ...
    if resources:
        ...
        st.success(f"🧰 Selected resources: **{selected_resource_labels}**")
    else:
        st.warning("Please select at least one resource for a more accurate recommendation.")
    ...
    if st.button(tr("generate"), type="primary", use_container_width=True):
        ...
        for rank, item in enumerate(top, start=1):
            ...
            with left:
                ...
                if animal_advice:
                    st.markdown("#### 🐄 Animal & Breed Advisory")
                    st.info(f"**Animal:** {animal_advice['animal']}\n\n**Location-based breed suggestion:** {animal_advice['breed']}\n\n**Why this suggestion:** {animal_advice['why']}")
                    st.markdown("**Basic care suggestions:**")
                    for care_item in animal_advice["care"]:
                        st.write("• " + care_item)
                    st.caption("Breed suggestions are regional prototype guidance. Confirm breed availability, suitability, vaccination and treatment plans with the local veterinary/Animal Husbandry department.")
        ...
        st.markdown("### 📊 Business Success & Risk Outlook")
        st.caption("These are prototype planning estimates, not historical success rates or guaranteed predictions.")
        o1, o2, o3 = st.columns(3)
        o1.metric("Estimated success potential", f"{lower}–{upper}%")
        o2.metric("Likely first positive results", outlook["success_period"])
        o3.metric("Higher-risk period", outlook["risk_period"])
        st.info(f"📈 **Stabilisation period:** {outlook['stable_period']}. Actual results depend on demand, costs, weather, water, management, competition and market prices.")
...
if page == "Business Recommendation":
    st.markdown("---")
    st.title("🌱 Seasonal & Location-Based Farming Advisor")
    st.write("The prototype recommends crops using the selected Karnataka location, season and a rule-based suitability score.")

    season_options = ["Kharif", "Rabi", "Summer"]
    selected_season = st.selectbox("🌦️ Farming season", season_options, index=season_options.index(current_season()))
    local_water = location_water(location)

    water_labels = {
        "Good": "Good", "Medium": "Medium", "Limited": "Limited"
    }
    st.info(f"📍 Location: **{location}**  |  🌦️ Season: **{selected_season}**  |  💧 Water context: **{water_labels[local_water]}**")

    recommendations = best_crops_for_location(location, selected_season, limit=5)

    if recommendations:
        st.subheader("🌾 Recommended crops for this location and season")
        st.caption("Prototype suitability score only — not a guaranteed yield or profit prediction. Confirm soil, irrigation, weather and local agriculture advice before planting.")
        ...
        for rank, (crop, base_score, info) in enumerate(recommendations, start=1):
            ...
            st.markdown(f"### #{rank} {tr_data(crop)} — {suitability}% suitability")
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Season", ", ".join(info["seasons"]))
            c2.metric("Water need", info["water"])
            c3.metric("Harvest window", info["harvest"])
            c4.metric("Base potential", f"{info['success']}%")
            st.write(f"**Why here:** {location} is included in the prototype suitability profile for this crop during the selected season.")
            st.write(f"**Main risk:** {info['risk']}")
            st.divider()
    else:
        st.warning("No strong prototype crop match was found for this location and season. Try another season and verify with local agricultural guidance.")

    st.info("💡 Example: the prototype can rank a water-demanding commercial crop such as sugarcane higher in locations with a suitable water profile, while recommending drought-tolerant crops such as tur, jowar or bengal gram for drier northern Karnataka locations. This is a rule-based demonstration, not a claim that one crop can never grow in another location.")
