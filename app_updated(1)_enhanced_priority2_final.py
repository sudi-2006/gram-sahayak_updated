...
P2_TR = {
    "English": {
        ...
        "animal_care":"Core care checklist", "market_link":"Market + location context", "next":"Suggested next step", "map_open":"Open map below", "no_weather":"Weather data unavailable right now.", "risk":"Risk flag", "low":"Low", "medium":"Medium", "high":"High"
    },
    "Hindi": {
...
if page == "Rural Intelligence Hub":
    ...
    with hub_tabs[0]:
        ...
        if wx["ok"]:
            ...
            for i,date in enumerate(daily["time"]):
                rows.append({"Date":date,"Weather":f"{weather_icon(daily['weather_code'][i])} {weather_text(weather_code_text(daily['weather_code'][i]))}","Min °C":round(daily['temperature_2m_min'][i],1),"Max °C":round(daily['temperature_2m_max'][i],1),"Rain %":daily['precipitation_probability_max'][i],"Rain mm":round(daily['precipitation_sum'][i],1)})
    ...
    with hub_tabs[1]:
        ...
        st.markdown(f'''<div class="feature-card"><div class="feature-icon">🌾</div><h4>{ui_data(location,"location_name")}: {s}/100</h4><p><b>{p2('local_crops')}:</b> {top_crop} and {max(local-1,0)} other profile(s).</p><p>{water_msg}</p><p><b>{p2('next')}:</b> Open Market Prices + Smart Action Plan to validate the opportunity with current inputs.</p></div>''', unsafe_allow_html=True)
        st.dataframe(map_df, use_container_width=True, hide_index=True)

    # ---------------- VOICE ----------------
    with hub_tabs[2]:
        page_banner("🎙️", p2("voice"), p2("voice_desc"))
        lang_code={"English":"en-IN","Hindi":"hi-IN","Kannada":"kn-IN"}[language]
        voice_texts={
            "English":{"idle":"Tap the microphone and speak.","not":"Speech recognition is not supported by this browser.","heard":"I heard:","responses":{"market":"Open Market Prices to review the demonstration crop prices for your selected location.","weather":"Open Weather & Farming Advisory for the latest available forecast.","crop":"Smart Action Plan can show the top prototype crops for your selected location and season.","scheme":"Government Schemes contains scheme eligibility and document guidance.","water":f"The prototype water context for {location} is {location_water(location)}."}},
            "Hindi":{"idle":"माइक्रोफोन दबाकर बोलें।","not":"इस ब्राउज़र में स्पीच रिकग्निशन समर्थित नहीं है।","heard":"मैंने सुना:","responses":{"market":"बाज़ार भाव खोलकर चयनित स्थान की प्रदर्शन फसल कीमतें देखें।","weather":"नवीनतम उपलब्ध पूर्वानुमान के लिए मौसम और खेती सलाह खोलें।","crop":"स्मार्ट कार्य योजना आपके स्थान और मौसम के लिए प्रोटोटाइप फसलें दिखा सकती है।","scheme":"सरकारी योजनाओं में पात्रता और दस्तावेज़ मार्गदर्शन उपलब्ध है।","water":f"{location} के लिए प्रोटोटाइप पानी संदर्भ {ui_data(location_water(location),'water')} है।"}},
            "Kannada":{"idle":"ಮೈಕ್ರೋಫೋನ್ ಒತ್ತಿ ಮಾತನಾಡಿ.","not":"ಈ ಬ್ರೌಸರ್‌ನಲ್ಲಿ ಸ್ಪೀಚ್ ರೆಕಗ್ನಿಷನ್ ಬೆಂಬಲಿತವಿಲ್ಲ.","heard":"ನಾನು ಕೇಳಿದ್ದು:","responses":{"market":"ಮಾರುಕಟ್ಟೆ ಬೆಲೆ ತೆರೆಯಿರಿ ಮತ್ತು ಆಯ್ಕೆ ಮಾಡಿದ ಸ್ಥಳದ ಪ್ರದರ್ಶನ ಬೆಳೆ ಬೆಲೆಗಳನ್ನು ನೋಡಿ.","weather":"ಲಭ್ಯವಿರುವ ಇತ್ತೀಚಿನ ಮುನ್ಸೂಚನೆಗಾಗಿ ಹವಾಮಾನ ಮತ್ತು ಕೃಷಿ ಸಲಹೆ ತೆರೆಯಿರಿ.","crop":"ಸ್ಮಾರ್ಟ್ ಕಾರ್ಯ ಯೋಜನೆ ನಿಮ್ಮ ಸ್ಥಳ ಮತ್ತು ಋತುವಿನ ಪ್ರೋಟೋಟೈಪ್ ಬೆಳೆಗಳನ್ನು ತೋರಿಸುತ್ತದೆ.","scheme":"ಸರ್ಕಾರಿ ಯೋಜನೆಗಳಲ್ಲಿ ಅರ್ಹತೆ ಮತ್ತು ದಾಖಲೆ ಮಾರ್ಗದರ್ಶನ ಇದೆ.","water":f"{location} ಸ್ಥಳದ ಪ್ರೋಟೋಟೈಪ್ ನೀರಿನ ಸಂದರ್ಭ {ui_data(location_water(location),'water')} ಆಗಿದೆ."}}
        }
        vt=voice_texts[language]
        payload=json.dumps(vt,ensure_ascii=False).replace("</","<\\/")
        html=f'''<div class="voice-card"><div class="voice-mic">🎙️</div><h3 style="text-align:center;margin-top:1rem">Gram Sahayak Voice</h3><p style="text-align:center">{p2("voice_hint")}</p><div style="display:flex;gap:10px;justify-content:center;margin:1rem 0"><button id="start" style="padding:12px 18px;border-radius:12px;border:1px solid #ddd;background:#fff;cursor:pointer">🎙️ {p2("voice_start")}</button><button id="stop" style="padding:12px 18px;border-radius:12px;border:1px solid #ddd;background:#fff;cursor:pointer">⏹️ {p2("voice_stop")}</button></div><div id="status" style="text-align:center;color:#667085">{vt['idle']}</div><div id="heard" style="margin-top:1rem;padding:1rem;background:#fff;border-radius:14px;border:1px solid #eee"></div><div id="reply" style="margin-top:1rem;padding:1rem;background:#fff;border-radius:14px;border:1px solid #eee"><b>{p2('voice_reply')}</b><br>—</div></div><script>const SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;const lang='{lang_code}';const texts={payload};let rec=null;if(!SpeechRecognition){{document.getElementById('status').innerText='{vt['not']}';}}else{{rec=new SpeechRecognition();rec.lang=lang;rec.continuous=false;rec.interimResults=false;rec.onstart=()=>document.getElementById('status').innerText='🎧 Listening...';rec.onend=()=>document.getElementById('status').innerText=texts.idle;rec.onerror=e=>document.getElementById('status').innerText='Microphone error: '+e.error;rec.onresult=e=>{{const said=e.results[0][0].transcript;document.getElementById('heard').innerHTML='<b>{vt['heard']}</b> '+said;const q=said.toLowerCase();let r=texts.responses.market;if(q.includes('weather')||q.includes('मौसम')||q.includes('ಹವಾಮಾನ'))r=texts.responses.weather;else if(q.includes('crop')||q.includes('फसल')||q.includes('ಬೆಳೆ'))r=texts.responses.crop;else if(q.includes('scheme')||q.includes('योजना')||q.includes('ಯೋಜನೆ'))r=texts.responses.scheme;else if(q.includes('water')||q.includes('पानी')||q.includes('ನೀರು'))r=texts.responses.water;document.getElementById('reply').innerHTML='<b>{p2('voice_reply')}</b><br>'+r;try{{const u=new SpeechSynthesisUtterance(r);u.lang=lang;window.speechSynthesis.speak(u);}}catch(err){{}}}};document.getElementById('start').onclick=()=>rec.start();document.getElementById('stop').onclick=()=>rec.stop();}}</script>'''
        components.html(html,height=460,scrolling=False)
        st.caption(p2("voice_browser"))
    ...
    with hub_tabs[3]:
        ...
        if margin>0:
            ...
            st.warning("⚠️ The current illustrative assumptions do not cover monthly operating cost. Review inputs before scaling.")
        if profile:
...
if page == "Home":
    ...
    st.markdown("### ✨ One assistant, four layers")
    cards=[("📈","Market Intelligence","Location-aware demonstration crop profiles and transparent sample pricing."),("🌦️","Live Context","Weather + season + water context for practical farming decisions."),("💰","Financial Planning","Profit, EMI and scenario simulation before spending money."),("🧠","Action Engine","Recommendations, risk alerts, comparisons and scheme readiness."),("🗺️","Opportunity View","Visual Karnataka opportunity signals built from the app's prototype profiles."),("🎙️","Voice Access","Browser voice interaction designed for accessibility and local-language use.")]
    ...
    st.markdown("### 🔄 How the system connects the dots")
    steps=[("01","Choose location","Karnataka location becomes the local context."),("02","Read signals","Market, water, season and weather are combined."),("03","Compare options","Crops, businesses and schemes can be compared."),("04","Plan before acting","Financial and livestock simulations expose assumptions."),("05","Take the next step","The Smart Action Plan turns information into practical actions.")]
    ...
    st.markdown("### 🚀 Demo flow for judges")
    st.success("Select a Karnataka location → open Rural Intelligence Hub → show live weather → show opportunity map → demonstrate voice → run Smart Action Plan → compare two options → check scheme documents.")
    st.caption("Prototype transparency: sample market data, rule-based recommendation logic and illustrative financial/livestock assumptions are clearly labelled inside the app.")
...
if page == "Market Prices":
    ...
    st.write(tr("market_desc", location=ui_data(location, "location_name")))

    local_crop_names = local_crops_for_location(location)

    st.markdown(f"### 🌾 {tr('local_crops_title')}")
    st.info(
        f"📍 **{location}** → " +
        ", ".join(tr_data(crop) for crop in local_crop_names)
    )
    st.caption(tr("local_crops_note"))

    show_all = st.checkbox(tr("show_all_crops"), value=False)

    available_crop_names = list(CROP_DATA.keys()) if show_all else local_crop_names
    categories = sorted(set(CROP_DATA[crop]["category"] for crop in available_crop_names))
    ...
    filtered = {
        crop: CROP_DATA[crop]
        for crop in available_crop_names
        if category == "All" or CROP_DATA[crop]["category"] == category
    }

    st.markdown(f"### 🧺 {tr('board')}")
    st.caption(tr("showing", count=len(filtered), location=ui_data(location, "location_name")))
    ...
    for start in range(0, len(items), 4):
        ...
        for col, (crop_name, data) in zip(cols, items[start:start + 4]):
            with col:
                adjusted = location_crop_price(crop_name, location)
                trend_icon = "📈" if data["trend"] == "Increasing" else ("📉" if data["trend"] == "Decreasing" else "➡️")
                local_tag = " • " + ui("local_crop") if crop_name in local_crop_names else ""
                st.markdown(f"""
                <div class="card">
                    <h4>🌾 {tr_data(crop_name)}</h4>
                    <p><b>{tr_data(data["category"])}</b>{local_tag}</p>
    ...
    if filtered:
        crop = st.selectbox(
            tr("choose_crop"),
            list(filtered.keys()),
            format_func=lambda x: tr_data(x)
        )
        selected = filtered[crop]
        sample_price = location_crop_price(crop, location)
        ...
        if crop in local_crop_names:
            st.success(f"🌱 {tr_data(crop)} {ui('local_profile', location=ui_data(location, 'location_name'))}.")
        else:
            st.info(f"ℹ️ {tr_data(crop)} {ui('show_all_reason')}")
    else:
        st.warning(ui("no_category"))
